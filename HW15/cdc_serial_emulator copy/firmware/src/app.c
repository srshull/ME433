...
void APP_Initialize ( void )
{
    ...
    TRISBbits.TRISB2 = 0; // RB2 is an output (motor 1)
    TRISBbits.TRISB3 = 0; // RB3 is an output (motor 2)
    
    LATAbits.LATA4 = 1; // RA4 is set high
    LATBbits.LATB2 = 1; // RB2 is set high (motor 1)
    LATBbits.LATB3 = 1; // RB3 is set high (motor 2)
	...
	PR2=47999; // gives us 1kHz frequency
	...
	OC1RS= 47999; // sets the future duty cycle
    RPA0Rbits.RPA0R = 0b0101; //OC1 on A0
	OC1CONbits.ON=1; // output compare 1 on
    
    OC2CONbits.OCTSEL=0; // use timer 2 for output compare
	OC2CONbits.OCM=6; // simple PWM mode, no fault pin
	OC2R= 24000; // sets the duty cycle
	OC2RS= 47999; // sets the future duty cycle
    RPA1Rbits.RPA1R = 0b0101; //OC2 on A1
	OC2CONbits.ON=1; // output compare 1 on
