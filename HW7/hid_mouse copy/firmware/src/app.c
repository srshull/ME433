...
void i2c_master_setup(void) {
  I2C2BRG = 0x34;            // 0XE9 gives 100kHz, 0x34 gives 400 kHz 
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            if(appData.emulateMouse)
            {
                ...
                if(movement_length > 50)
                {
                    ...
                    i2c_master_send(0x2A); // send address of Y_LOW register for accelerometer
                    i2c_master_restart(); // send a RESTART so we can begin reading 
                    i2c_master_send((IMU_ADDR << 1) | 1); //1 in LSB indicating read
                    Y_low = i2c_master_recv(); // receive a byte from the bus
                    i2c_master_ack(0); // send ACK - want another byte
                    Y_high = i2c_master_recv(); // receive a byte from the bus
                    i2c_master_ack(1); // send NACK - don't want another byte
                    i2c_master_stop(); // send stop
                    Y_full = (Y_high << 8 | Y_low);
                    //appData.xCoordinate =(int8_t)dir_table[vector & 0x07] ;
                    appData.yCoordinate = Y_full/100;
                    vector ++;
                    movement_length = 0;
                }
            }
